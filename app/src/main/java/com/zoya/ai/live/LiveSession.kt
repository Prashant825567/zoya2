...
class LiveSession(
    ...
    private val languageInstruction: String = "",
    ...
    val micMuted: Boolean get() = _micMuted

    fun setMicMuted(muted: Boolean) { _micMuted = muted }
    ...
    private fun sendSetupMessage() {
        ...
        val fullSystemInstruction = buildString {
            ...
            append("\n\nThe user's name is ")
            append(userName)
            append(". Use their name sometimes to make it personal.\n")
            append("You can open websites, get current time, and search Google when asked.\n")
            append("Keep responses conversational and natural for voice.\n\n")
        ...
        val msg = JSONObject().apply {
            put("setup", JSONObject().apply {
                ...
                put("systemInstruction", JSONObject().apply {
                    put("parts", JSONArray().put(JSONObject().apply { put("text", fullSystemInstruction) }))
                })
        ...
        recordingJob = scope.launch {
            try {
                audioRecorder.startRecording().collect { bytes ->
                    if (webSocket != null && _state.value != SessionState.DISCONNECTED && !_micMuted) {
